...
export default function Step1_DeliveryForm({
  formData,
  setFormData,
  errors,
  setErrors,
  ...
  return (
    <form className={styles.deliveryForm}>
      ...
      <div className={styles.formGroup}>
        <label htmlFor="email" className={errors.email ? styles.errorLabel : ""}>
          E-Posta*
        </label>
        <input
          type="email"
          ...
          className={`${styles.checkoutInput} ${errors.email ? styles.errorInput : ""}`}
        />
      ...
      {fields.map(({ name, label, type, placeholder }) => (
        <div key={name} className={styles.formGroup}>
          <label htmlFor={name} className={errors[name] ? styles.errorLabel : ""}>
            {label}
          </label>
          ...
          {type === "select" && name === "country" ? (
            <select
              ...
              disabled
              className={`${styles.checkoutSelect} ${errors[name] ? styles.errorInput : ""}`}
            >
            ...
            <textarea
              ...
              onChange={handleChange}
              rows={3}
              className={`${styles.checkoutTextarea} ${errors[name] ? styles.errorInput : ""}`}
            />
          ) : (
            <input
              name={name}
              ...
              className={`${styles.checkoutInput} ${errors[name] ? styles.errorInput : ""}`}
            />
          )}
